    ...
    <section id="about" aria-labelledby="about-title" className="relative scroll-mt-[72px] py-14 lg:scroll-mt-0 lg:py-20">
      <div className={cn(container, 'relative z-[4] lg:grid lg:grid-cols-[72px_380px_minmax(0,1fr)] lg:gap-x-10')}>
        ...
        <div className="mt-5 lg:mt-0">
          <p className="max-w-[34ch] text-xl leading-[1.55] lg:text-[22px]">{lead}</p>
          <p className="mt-[18px] max-w-[34ch] text-ink-2">{detail}</p>

          <div className="mt-[30px] flex items-start gap-4 border-[1.5px] border-dashed border-ink px-5 py-[18px]">
            <span
              aria-hidden="true"
              className="mt-[3px] size-5 shrink-0 rounded-full border-[3px] border-dashed"
          ...
          <dl className="mt-[34px] border-t-[1.5px] border-ink">
            {copy.about.facts.map((fact) => (
              <div key={fact.label} className="grid grid-cols-[130px_1fr] gap-3 border-b border-rule py-[13px] text-base lg:grid-cols-[150px_1fr]">
